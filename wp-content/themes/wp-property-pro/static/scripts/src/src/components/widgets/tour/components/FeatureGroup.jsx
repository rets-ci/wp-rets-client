...
const FeatureGroup = ({browserHistoryPush, featureGroup, ind, openFormModal}) => {
  ...
  let counter = 1;
  const featuresCount = get(featureGroup, 'features', []).length;
  const backgroundPosition = get(featureGroup, 'layout', null)
  const backgroundSize = get(featureGroup, 'background', null)
  ...
  let backgroundStyle = get(featureGroup, 'image_section.image_src', null) !== null
    ? {
              ...
              <div className="row no-gutters">
                <div className={featureGroupContentClasses}>
                  <div className={Lib.THEME_CLASSES_PREFIX + "tour-widget-content"}>
                    {
                      get(featureGroup, 'features', []).map((feature, k) => {
