...
class class_agents {
  ...
  static public function add_filter_field( $fields ) {
    ...
    $fields[] = array(
        'id' => 'wpp_agents',
        'name' => __( 'Agent', ud_get_wpp_agents()->domain ),
  ...
  static public function add_capability($capabilities) {
    ...
    $capabilities[self::$capability] = __('Manage Real Estate Agents',ud_get_wpp_agents()->domain);
  ...
  */
  ...
  ?>
      <li>
        <label class="description"><?php echo __('Match the "Property Agent" attribute to ',ud_get_wpp_agents()->domain);?></label>
  ...
  */
  static public function flyer_settings($wpp_pdf_flyer) {
    ...
    ?>
    <tr>
      <th><?php _e("Agent Image Size", ud_get_wpp_agents()->domain); ?></th>
      <td>
  ...
  */
  static public function pdf_flyer_insert( $property, $wpp_pdf_flyer) {
    ...
    if(!empty($wpp_pdf_flyer['pr_agent_info']) && !empty($property['wpp_agents'])) {
      ...
      $agent_photo_width = $wpp_pdf_flyer['agent_photo_width'];
        ?>
        <tr>
            <td><div class="heading_text"><?php echo __('Agent Information', ud_get_wpp_agents()->domain); ?></div>
            </td>
        </tr>
        <tr>
            <td><br/>
  ...
  static public function metabox_save($user) {
    ...
    <div id="major-publishing-actions">
      <div id="publishing-action">
        <input type="submit" id="publish" value="<?php if (isset($user->new_user) && $user->new_user) _e('Create Agent', ud_get_wpp_agents()->domain); else _e('Update Agent', ud_get_wpp_agents()->domain); ?>" class="button-primary btn" id="submit" name="submit" />
      </div>
    </div>
    <?php
  }
        ...
        <tr>
          ...
          <td>
            <ul>
              ...
              <li><span class="description"><?php _e('Content that is displayed in the agent widget.', ud_get_wpp_agents()->domain); ?></span></li>
            </ul>
          </td>
        </tr>
  ...
  static public function settings_nav($tabs) {
    $tabs['agents'] = array(
      'slug' => 'agents',
      'title' => __('Agents',ud_get_wpp_agents()->domain)
    );
    return $tabs;
  }
  ...
  static public function admin_menu() {
    ...
    if(!empty($wp_properties['labels']['agents']['plural'])) {
     ...
     $label = __('Agents', ud_get_wpp_agents()->domain);
   }
  ...
  static public function show_agents() {
    ...
    <h2><?php _e('Agents', ud_get_wpp_agents()->domain); ?> <a class="button add-new-h2" href="<?php echo self::get_link_edit(-1); ?>"><?php _e('Add New', ud_get_wpp_agents()->domain); ?></a></h2>
...
?>
    ...
    <div id="profile-page" class="wpp_agent_page wrap">
      ...
      <?php if (isset($_REQUEST['status']) &&  $_REQUEST['status'] == 'create' ) : ?>
        <div class="updated fade" id="message"><p><?php _e('Agent created.', ud_get_wpp_agents()->domain); ?></p></div>
      <?php endif; ?>

      <h2><?php _e('Edit Agent', ud_get_wpp_agents()->domain); ?> <?php echo $user->display_name; ?></h2>
  ...
  static public function publish_box_options() {
    ...
    <li class="wpp_agent_select_wrap">
      <span class="wpp_agent_selector_title"><?php _e('Associated Agents', ud_get_wpp_agents()->domain); ?></span>
