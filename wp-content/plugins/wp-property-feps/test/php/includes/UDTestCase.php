...
class UD_Plugin_WP_UnitTestCase extends WP_UnitTestCase {
  ...
  function setUp() {
    ...
    if( file_exists( $this->root_dir . '/wp-property-feps.php' ) ) {
      include_once( $this->root_dir . '/wp-property-feps.php' );
    }
    if( !class_exists( '\UsabilityDynamics\WPP\FEPS_Bootstrap' ) ) {
      $this->fail( 'Plugin is not available.' );
    }
    $this->instance = \UsabilityDynamics\WPP\FEPS_Bootstrap::get_instance();
  }
