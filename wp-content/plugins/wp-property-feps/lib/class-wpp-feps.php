...
class class_wpp_feps {
    ...
    <table class="form-table">
      <tbody>
        ...
        <tr>
          ...
          <td>
            <?php printf( __( 'All FEPS notifications can be customized via <a href="%s">WP-CRM</a> plugin. For more details, look through <b>FEPS -> Integration with other UsabilityDynamics plugins</b> section in Help tab above.', ud_get_wpp_feps()->domain ), 'https://usabilitydynamics.com/products/wp-invoice/' ); ?>
          </td>
        </tr>
      </tbody>
    </table>
  ...
  static public function contextual_help_property_forms() {
    ...
    $contextual_help[ $tabs['plans'] ][] = '<p>' . sprintf( __( 'The current functionality is available only for <b>sponsored listings</b> ( see "%s" tab ) when <a href="%s">WP-Invoice</a> plugin and <a href="%s">Single Page Checkout (SPC)</a> plugin are installed and activated.', ud_get_wpp_feps()->domain ), $tabs['options'], 'https://usabilitydynamics.com/products/wp-invoice/', 'https://usabilitydynamics.com/products/wp-invoice-spc/' ) . '</p>';
  ...
  static public function contextual_help_settings( $contextual_help ) {
    ...
    $contextual_help[ $tab ][] = sprintf( __( 'It also allows to create forms for sponsored listings if you have installed <a href="%s">WP-Invoice</a> plugin and <a href="%s">Single Page Checkout (SPC)</a> plugin.', ud_get_wpp_feps()->domain ), 'https://usabilitydynamics.com/products/wp-invoice/', 'https://usabilitydynamics.com/products/wp-invoice-spc/' ) . '</p>';
    ...
    $contextual_help[ $tab ][] = sprintf( __( 'Functionality of FEPS premium feature can increased via other following wordpress plugins developed by <a href="%s">UsabilityDynamics, Inc</a>:', ud_get_wpp_feps()->domain ), 'https://usabilitydynamics.com/' ) .'<ul>';
    $contextual_help[ $tab ][] = '<li>' . sprintf( __( '<a href="%s">WP-Invoice</a>', ud_get_wpp_feps()->domain ), 'https://usabilitydynamics.com/products/wp-invoice/' ) .'</li>';
    $contextual_help[ $tab ][] = '<li>' . sprintf( __( '<a href="%s">WP-CRM</a>', ud_get_wpp_feps()->domain ), 'https://usabilitydynamics.com/products/wp-crm/' ) .'</li></ul>';
    $contextual_help[ $tab ][] = '<h5>' . __( 'WP-Invoice', ud_get_wpp_feps()->domain ) .'</h5>';
    $contextual_help[ $tab ][] = '<p>' . sprintf( __( 'To add aditional functionality to FEPS premium feature, <a href="%s">WP-Invoice Single Page Checkout</a> plugin must be installed and activated.', ud_get_wpp_feps()->domain ), 'https://usabilitydynamics.com/products/wp-invoice-spc/' ) . '</p>';
      ...
      <script type="text/javascript">
        jQuery(document).ready(function() {
          if ( jQuery('#publish').length > 0 ) {
            jQuery( '.wrap h2' ).after( '<div class="wpp_property_admin_notice widget wp-core-ui"><span><?php _e('This listing is pending your approval.', ud_get_wpp_feps()->domain); ?></span></div>' );
  ...
  static public function wpp_get_search_filters ( $filter ){
    ...
    if ( !empty( $values ) ) {
      $filter[ FEPS_META_FORM ] = array (
        'type' => 'multi_checkbox',
        'label' => sprintf(__('%1$s Forms',ud_get_wpp_feps()->domain), ucfirst( WPP_F::property_label( 'singular' ) )),
        'values' => (array)$values
      );
    }
      ...
      <ul>
        ...
        <?php else : ?>
          <li class="wpp_feps_all_listings">
            <a href="<?php echo  WPP_F::base_url( FEPS_VIEW_PAGE ); ?>"><?php printf( __("My %s", ud_get_wpp_feps()->domain), WPP_F::property_label( 'plural' ) ); ?></a>
          </li>
        <?php endif; ?>
      </ul>
  ...
  static public function spc_options_general_miss( $args = '' ) {
    ...
    <li>
      ...
      &nbsp;<span class="description">Get <a href="https://usabilitydynamics.com/products/wp-invoice-spc/">WP-Invoice Single Page Checkout (SPC) plugin</a> for <a class="small" href="<?php echo admin_url('plugin-install.php?tab=search&amp;type=term&amp;s=wp-invoice+billing+andypotanin');?>">WP-Invoice</a> to use FEPS for Sponsored Listings.</span>
    </li>
  <?php
  }
  ...
  static public function feps_user_activation() {
    ...
    if ( $is_not_approved && md5($user->ID.$user->data->user_email.SECURE_AUTH_SALT) == $_REQUEST['wpp_user_activation'] ) {
      ...
      wp_die( sprintf( __('Thank you! Your account has been activated. <a href="%s">Back to the site!</a>', ud_get_wpp_feps()->domain), get_bloginfo('home') ), __('Account has been activated', ud_get_wpp_feps()->domain));
    } else {
