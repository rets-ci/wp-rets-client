...
class UD_Plugin_WP_UnitTestCase extends WP_UnitTestCase {
  ...
  function setUp() {
    ...
    if( file_exists( $this->root_dir . '/wpp-walkscore.php' ) ) {
      include_once( $this->root_dir . '/wpp-walkscore.php' );
    }
    if( !class_exists( '\UsabilityDynamics\WPP\WS_Bootstrap' ) ) {
      $this->fail( 'Plugin is not available.' );
    }
    $this->instance = \UsabilityDynamics\WPP\WS_Bootstrap::get_instance();
  }
