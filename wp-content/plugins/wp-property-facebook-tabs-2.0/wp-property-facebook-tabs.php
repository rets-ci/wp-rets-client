<?php
/**
 * Plugin Name: WP-Property: Facebook Tabs
 * Plugin URI: https://usabilitydynamics.com/product/wp-property-facebook-tabs
...
if( !function_exists( 'ud_check_wpp_fbtabs' ) ) {
  ...
  function ud_check_wpp_fbtabs() {
    ...
    try {
      ...
      $_ud_wpp_fbtabs_error = $e->getMessage();
      return false;
    }
    return true;
  }
...
if( !function_exists( 'ud_my_wp_plugin_message' ) ) {
   ...
   * @since 1.0.0
   */
  function ud_wpp_fbtabs_message() {
    global $_ud_wpp_fbtabs_error;
    if( !empty( $_ud_wpp_fbtabs_error ) ) {
      $message = sprintf( __( '<p><b>%s</b> can not be initialized. %s</p>', 'wpp_fbtabs' ), 'WP-Property Facebook Tabs', $_ud_wpp_fbtabs_error );
      echo '<div class="error fade" style="padding:11px;">' . $message . '</div>';
    }
  }
  add_action( 'admin_notices', 'ud_wpp_fbtabs_message' );
}
