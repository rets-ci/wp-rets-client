...
class UD_Plugin_WP_UnitTestCase extends WP_UnitTestCase {
  ...
  function setUp() {
    ...
    if( file_exists( $this->root_dir . '/wp-property-facebook-tabs.php' ) ) {
      include_once( $this->root_dir . '/wp-property-facebook-tabs.php' );
    }
    if( !class_exists( '\UsabilityDynamics\WPP\Facebook_Tabs_Bootstrap' ) ) {
      $this->fail( 'Plugin is not available.' );
    }
    $this->instance = \UsabilityDynamics\WPP\Facebook_Tabs_Bootstrap::get_instance();
  }
