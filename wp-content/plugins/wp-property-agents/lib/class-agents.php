...
class class_agents {
  ...
  */
  ...
  ?>
      <li>
        <label class="description"><?php printf( __('Match the "%s %s" attribute to ',ud_get_wpp_agents()->domain), WPP_F::property_label(), ud_get_wp_property( 'configuration.feature_settings.agents.label.single' ) );?></label>
        <select  name="wpp_property_import[wpp_agent_attribute_match]">
        <?php foreach( $agent_fields as $field_slug => $field_title ) { ?>
          <option value="<?php echo esc_attr( $field_slug ); ?>" <?php if( isset( $current_settings[ 'wpp_agent_attribute_match' ] ) ) selected( $field_slug, $current_settings[ 'wpp_agent_attribute_match' ] ); ?>><?php echo $field_title; ?></option>
        <?php } ?>
        </select>
        <label  class="description"><?php echo __(' field, when it is found, and associate with agent.',ud_get_wpp_agents()->domain);?></label>
    </li>
  <?php
  ...
  */
  static public function pdf_flyer_insert( $property, $wpp_pdf_flyer) {
    ...
    if(!empty($wpp_pdf_flyer['pr_agent_info']) && !empty($property['wpp_agents'])) {
      ...
      $agent_photo_width = $wpp_pdf_flyer['agent_photo_width'];
        ?>
        <tr>
            <td><div class="heading_text"><?php printf( __('%s Information', ud_get_wpp_agents()->domain), ud_get_wp_property( 'configuration.feature_settings.agents.label.single' ) ); ?></div>
            </td>
        </tr>
        <tr>
            <td><br/>
            ...
            <table cellspacing="0" cellpadding="0" border="0">
              <tr>
                ...
                <td width="<?php echo $agent_photo_width; ?>"><?php
                  ...
                  if( preg_match('@^(https?|ftp)://[^\s/$.?#].[^\s]*$@iS',$agent_images[0]['src']) ) { ?><table cellspacing="0" cellpadding="10" border="0" class="bg-section">
                    <tr>
  ...
  static public function manage_users_custom_column($nothing, $column_name, $user_id) {
    ...
    switch($column_name) {
      ...
      case 'display_name':
        ...
        <span class="edit"><a href="<?php echo self::get_link_edit($user_id); ?>"><?php _e('Edit', ud_get_wpp_agents()->domain); ?></a> | </span>
        <span class="delete"><a href="<?php echo self::get_link_delete($user_id); ?>" class="submitdelete"><?php _e('Delete', ud_get_wpp_agents()->domain); ?></a></span>
        </div>
        <?php
  ...
  static public function metabox_save($user) {
    ...
    <div id="major-publishing-actions">
      <div id="publishing-action">
        <input type="submit" id="publish" value="<?php if (isset($user->new_user) && $user->new_user) printf( __('Create %s', ud_get_wpp_agents()->domain), ud_get_wp_property( 'configuration.feature_settings.agents.label.single' ) ); else printf( __('Update %s', ud_get_wpp_agents()->domain),  ud_get_wp_property( 'configuration.feature_settings.agents.label.single' ) ); ?>" class="button-primary btn" id="submit" name="submit" />
      </div>
    </div>
    <?php
  }
        ...
        <tr>
          ...
          <td>
            <ul>
              ...
              <li><span class="description"><?php printf( __('Content that is displayed in the %s widget.', ud_get_wpp_agents()->domain), ud_get_wp_property( 'configuration.feature_settings.agents.label.plural' ) ); ?></span></li>
            </ul>
          </td>
        </tr>
        ...
        <tr>
          ...
          <td>
            <ul>
              ...
              <li><span class="description"><?php _e('Content that can be displayed in the shortcode.', ud_get_wpp_agents()->domain); ?></span></li>
            </ul>
          </td>
        </tr>
  ...
  static public function show_agents() {
    ...
    <h2><?php echo ud_get_wp_property( 'configuration.feature_settings.agents.label.plural' ); ?> <a class="button add-new-h2" href="<?php echo self::get_link_edit(-1); ?>"><?php _e('Add New', ud_get_wpp_agents()->domain); ?></a></h2>
...
?>
    ...
    <div id="profile-page" class="wpp_agent_page wrap">
      ...
      <?php if (isset($_REQUEST['status']) &&  $_REQUEST['status'] == 'create' ) : ?>
        <div class="updated fade" id="message"><p><?php printf( __('%s created.', ud_get_wpp_agents()->domain), ud_get_wp_property( 'configuration.feature_settings.agents.label.single' ) ); ?></p></div>
      <?php endif; ?>
  ...
  static public function publish_box_options() {
    ...
    <li class="wpp_agent_select_wrap">
      <span class="wpp_agent_selector_title"><?php printf( __('Associated %s', ud_get_wpp_agents()->domain), ud_get_wp_property( 'configuration.feature_settings.agents.label.plural' ) ); ?></span>
  ...
  static public function show_profile_fields($user) {
    ...
    <table class="form-table">
      <?php if ( is_array($fields) and !empty($fields) ) : ?>
        ...
        <?php endforeach; ?>
      <?php else: ?>
        <tr><td colspan="2"><?php printf(__('You can add fields on the <a href="%s">settings page</a>.', ud_get_wpp_agents()->domain), 'edit.php?post_type=property&page=property_settings#tab_agents'); ?></td></tr>
      <?php endif; ?>
      ...
      <?php if ( class_exists('class_wpp_pdf_flyer') ) : ?>
        <tr>
          ...
          <td>
            ...
            <span class="description"><?php _e('Please enter a writeup for flyers.', ud_get_wpp_agents()->domain); ?></span>
          </td>
        </tr>
      <?php endif; ?>
    </table>
  ...
  */
  static public function display_agent_card($agent_id, $args = '') {
    ...
    <div class='wpp_agent_info_single_wrapper'>
      ...
      <?php if( isset( $dl_print_fields ) && is_array( $dl_print_fields ) ): ?>
