...
class class_wpp_feps {
  ...
  static public function settings_page() {
    ...
    <table class="form-table">
      <tbody>
        <tr>
          <th><?php printf( __('%1s Forms','wpp'), WPP_F::property_label() );; ?></th>
          <td>
            <?php printf( __( '<a href="%s">Manage %s Forms', 'wpp' ), admin_url( 'edit.php?post_type=property&page=page_feps' ), WPP_F::property_label() ); ?>
          </td>
        </tr>
        <tr>
          ...
          <td>
            ...
            <ul>
              ...
              <li><?php echo WPP_F::input("name=wpp_settings[configuration][feps][menu][add_credits_label]&style=width:250px;", $config['menu']['add_credits_label'] ); ?> <?php _e( '"Add Credits to Balance" custom label', 'wpp' ); ?></li>
              <?php endif; ?>
              <li><?php echo WPP_F::input("name=wpp_settings[configuration][feps][menu][add_property_label]&style=width:250px;", $config[ 'menu' ]['add_property_label']); ?> <?php printf ( __('"Add New %s" custom label','wpp'), WPP_F::property_label() ); ?></li>
              <li><?php echo WPP_F::checkbox("name=wpp_settings[configuration][feps][menu][disable_if_no_properties]&label=" . sprintf( __('Disable Menu if there are no %s and no credits on Balance', 'wpp'), WPP_F::property_label( 'plural' ) ), $config[ 'menu' ]['disable_if_no_properties']); ?></li>
            </ul>
          </td>
        </tr>
        <tr>
          ...
          <td>
            ...
            <ul class="feps_specific_pages_links">
              ...
              <li class="feps_templates">
                <label><?php _e( 'Edit Page', 'wpp' ); ?>:</label>
  ...
  static public function contextual_help_property_forms() {
    ...
    $tabs = array(
      ...
      'attributes' => __( 'Propery Attributes', 'wpp'  ),
    );
    ...
    $contextual_help[ $tabs['shortcode'] ][] = '<li><b>detect_parent</b>: ' . sprintf( __( 'Available values "true" and "false". Default value is "true". If set to "true", the form will automatically detect if it is displayed on a %1$s page, and use the current %1$s as parent for newly added %2$s. If parent_id is set, detect parent does not do anything.', 'wpp' ), WPP_F::property_label(), WPP_F::property_label( 'plural' ) ) . '</li>';
    ...
    $contextual_help[ $tabs['plans'] ][] = '<h4>' . __( 'Is Featured', 'wpp' ) .'</h4>';
    $contextual_help[ $tabs['plans'] ][] = '<p>' . sprintf( __( '%s will be featured if option is checked.', 'wpp' ), WPP_F::property_label() ) .'</p>';
  ...
  static public function contextual_help_settings( $contextual_help ) {
    ...
    $contextual_help[ $tab ][] = '<br/>';
    $contextual_help[ $tab ][] = '<h4>' . __( 'FEPS Menu Widget/Shortcode', 'wpp' ) .'</h4>';
    ...
    $contextual_help[ $tab ][] = '</ul>';
    $contextual_help[ $tab ][] = '<br/>';
    $contextual_help[ $tab ][] = '<h4>' . __( 'FEPS Information Widget/Shortcode', 'wpp' ) .'</h4>';
    $contextual_help[ $tab ][] = '<p>' . __( 'It\'s a block which contains some specific information about current property.', 'wpp' ) .'</p>';
    $contextual_help[ $tab ][] = '<p>' . sprintf( __( 'It\'s only shown on Edit FEPS %s page for logged in users', 'wpp' ), WPP_F::property_label() ) .'</p>';
    $contextual_help[ $tab ][] = '<br/>';
    ...
    $contextual_help[ $tab ][] = '<p>' . sprintf( __( 'To add aditional functionality to FEPS premium feature, <a href="%s">Single Page Checkout</a> premium feature must be installed and activated.', 'wpp' ), 'https://usabilitydynamics.com/products/wp-invoice/premium-features/' ) . '</p>';
  ...
  static public function post_property_form_information() {
    ...
    if( !empty( $plan ) && is_array( $plan ) ) {
      ...
      $plan_details .= ', ' . __( 'Featured', 'wpp' ) . ': ' . ( !empty( $plan['is_featured'] ) && $plan['is_featured'] == 'true' ? __( 'Yes', 'wpp' ) : __( 'No', 'wpp' ) );
    }
          ...
          <tbody>
            ...
            <tr class="wpp_attribute_row_parent wpp_attribute_row">
              ...
              <?php if ( current_user_can( 'edit_users', $user->ID ) ) : ?>
                ...
                <span class="description">( <?php printf( __( 'The user has <b>%01.2f</b> credits on the balance', 'wpp' ), !empty( $credits ) ? $credits : 0 ); ?> )</span>
              <?php else : ?>
  ...
  static public function maybe_renew_subscripton_plan( $post_id ) {
    ...
    $subject = __( 'Renewal of Subscription Plan', 'wpp' );
    $message = sprintf(__('Hello [display_name]%1$s%1$sPublish time for Your %2$s "[property_title]" has expired.%1$s%1$sSubscription Plan has been renewed and [credits] credits were withdrawn from your balance.%1$s%1$sClick this link to view all your [status] %3$s:%1$s[url]', 'wpp'), PHP_EOL, WPP_F::property_label( 'singular' ), WPP_F::property_label( 'plural' ) );
    $crm_log_message = __( '', 'wpp' );
    WPP_Mail::feps_post_status_updated( $post->ID, array_filter( array(
      'subject' => $subject,
      'message' => $message,
      'crm_log_message' => $crm_log_message,
  ...
  static public function feps_edit_property_options( $property ) {
    ...
    <li class="<?php wpp_css("feps-default-template::row-wrapper", array("wpp_feps_row_wrapper", "enable_automatic_renewal")); ?>">
      <div class="<?php wpp_css("feps-default-template::label-wrapper", array("wpp_feps_label_wrapper")); ?>">
        <label><span class="<?php wpp_css("feps-default-template::the_title","the_title"); ?>"><?php _e('Options:', 'wpp'); ?></span></label>
      </div>
      <div class="<?php wpp_css("feps-default-template::input-wrapper","wpp_feps_input_wrapper"); ?>">
          <div class="<?php wpp_css("feps-default-template::input-content","wpp_feps_input_content"); ?>">
            <label><input id="renew_plan_<?php echo $post_id; ?>" type="checkbox" value="" <?php echo $renew_plan; ?> /> <?php printf( __( 'Enable automatic renewal of Subscription plan for the current %1$s when subscription time is expired.', 'wpp' ), WPP_F::property_label( 'singular' ) ); ?></label>
          </div>
      ...
      <script type="text/javascript">
        jQuery(document).ready(function() {
          if ( jQuery('#publish').length > 0 ) {
            jQuery( '.wrap h2' ).after( '<div class="wpp_property_admin_notice widget wp-core-ui"><span><?php _e('This listing is pending your approval.', 'wpp'); ?></span></div>' );
  ...
  static public function admin_enqueue_scripts() {
    ...
    switch($current_screen->id)  {
      ...
      case 'property_page_page_feps':
        wp_enqueue_script( 'wp-property-admin-feps', WPP_URL . 'js/wpp.admin.feps.js', array( 'jquery', 'wpp-localization' ), WPP_Version );
  ...
  static public function wpp_feps_info( $atts = '' ) {
    ...
    <div class="wpp_feps_property_info wpp_shortcode_feps_info wpp_shortcode wpp_shortcode_feps_menu wpp_feps_user_menu">
      ...
      <ul>
        <li>
          <span class="title"><?php _e("Title", 'wpp'); ?></span><span class="doubledot">:</span>
          <span class="value"><?php echo $post->post_title; ?></span>
        </li>
        <li>
          <span class="title"><?php _e("Status", 'wpp'); ?></span><span class="doubledot">:</span>
          <span class="value"><?php echo  WPP_F::clear_post_status( $post->post_status ); ?></span>
        </li>
        <?php if( !empty( $expired_time ) ): ?>
        <li>
          <span class="title"><?php _e("Expired", 'wpp'); ?></span><span class="doubledot">:</span>
          <span class="value"><?php echo $expired_time; ?></span>
        </li>
        <?php endif; ?>
        <?php if( !empty( $plan ) && is_array( $plan ) ) : ?>
        <li>
          <span class="title"><?php _e("Subscription Plan", 'wpp'); ?></span><span class="doubledot">:</span>
          <span class="value"><?php echo $plan[ 'name' ]; ?></span>
        </li>
        <li>
          <span class="title"><?php _e("Paid", 'wpp'); ?></span><span class="doubledot">:</span>
          <span class="value"><?php printf( __( '%01.2f credits', 'wpp' ), $plan[ 'price' ] ); ?></span>
        </li>
        <li>
          <span class="title"><?php _e("Plan Details", 'wpp'); ?></span><span class="doubledot">:</span><br/>
          <span><?php _e("Duration", 'wpp'); ?></span><span class="doubledot">:</span>
          <span class="value"><?php printf( '%d %s', $plan['duration']['value'], _n( $plan['duration']['interval'], $plan['duration']['interval'].'s', $plan['duration']['value'], 'wpp' ) ); ?></span><br/>
          <span><?php _e("Images Limit", 'wpp'); ?></span><span class="doubledot">:</span>
          <span class="value"><?php echo ( !empty( $plan['images_limit'] ) ? $plan['images_limit'] : '1' ); ?></span>
        </li>
        <?php elseif ( $sponsored ): ?>
        <li>
          <span class="title"><?php _e("Subscription Plan", 'wpp'); ?></span><span class="doubledot">:</span>
          <span class="value"><?php _e( 'Not Paid Yet', 'wpp' ); ?></span>
        </li>
        <?php endif; ?>
      </ul>
    </div>
  ...
  static public function page_feps() {
    ...
    $general_attributes = array( 'General' => array(
      'post_content' => sprintf( __( '%1$s Content', 'wpp' ), WPP_F::property_label( 'singular' ) ),
      'property_type' => sprintf( __( '%1$s Type', 'wpp' ), WPP_F::property_label( 'singular' ) ),
    ) );
            ...
            <tr>
              ...
              <td>
                ...
                <ul>
                  ...
                  <li>
                    <?php printf( __('%1$s Type:', 'wpp'), WPP_F::property_label() ); ?>
                    ...
                    <span class="description"><?php printf( __('Note: This type will be set by default if %1$s Type is not set in %1$s Attributes below.', 'wpp' ), WPP_F::property_label() ); ?></span>
                  </li>
                  <li>
                  ...
                  <?php if( class_exists( 'class_wpp_slideshow' ) ) : ?>
                    <li>
                      ...
                      <label for="auto_add_to_slideshow_<?php echo $form_id; ?>"><?php printf( __( 'Automatically load added images into %1$s Slideshow. ', 'wpp' ), WPP_F::property_label() ); ?></label>
                      <span class="description"><?php printf( __('Note: You can use [property_slideshow] shortcode on Single %1$s page to show added images. Also, some premium themes such as Denali support automatic slideshow view on single %1$s pages.', 'wpp' ), WPP_F::property_label() ); ?></span>
                    </li>
                  <?php endif; ?>
                  <li>
                    <input id="can_manage_feps_<?php echo $form_id; ?>" type="checkbox" name="wpp_feps[forms][<?php echo $form_id; ?>][can_manage_feps]" <?php if( $form['can_manage_feps'] ) checked('true', $form['can_manage_feps']); ?> value="true" />
                    <label for="can_manage_feps_<?php echo $form_id; ?>"><?php _e('Allow user to edit and remove his FEPS.', 'wpp'); ?></label>
                  ...
                  <li <?php echo $hidden; ?>>
                    ...
                    <label for="user_creation_<?php echo $form_id; ?>"><?php _e('Disable new user account creation notification.', 'wpp'); ?></label>
                  </li>
                  <li>
                    ...
                    <label for="on_status_updated_<?php echo $form_id; ?>"><?php _e('Notify user on update status of his FEPS.', 'wpp'); ?></label>
                  </li>
                </ul>
              </td>
            </tr>
              ...
              <th colspan="2">
                ...
                <table class="ud_ui_dynamic_table widefat wpp_feps_sortable" use_random_row_id="true">
                      ...
                      <td class="main_feps">
                        <ul>
                          ...
                          <li class="wpp_development_advanced_option is_required">
                            ...
                            <label for="required_<?php echo $form_id; ?>_<?php echo $row_id; ?>"><?php _e('Required', 'wpp'); ?></label>
                          </li>
                          <li class="wpp_development_advanced_option">
                            <a class="wpp_forms_remove_attribute" row="<?php echo $row_id; ?>" href="javascript:void(0);"><?php _e( 'Remove attribute', 'wpp' ); ?></a>
                          </li>
                          <li>
                            <span class="wpp_show_advanced"><?php _e('Toggle Advanced Settings', 'wpp'); ?></span>
                          </li>
                  ...
                  <tfoot>
                    <tr>
                      <td colspan="3">
                        <input type="button" callback_function="wpp.ui.feps.on_added_row" class="wpp_add_row button-secondary" value="<?php _e('Add Row','wpp') ?>" />
                      </td>
                    </tr>
                  </tfoot>
                </table>
  ...
  static public function wpp_feps_input( $data ) {
    ...
    if( isset( $att_data[ 'slug' ] ) && $att_data[ 'slug' ] == 'property_type' ) {
      $input_type = 'dropdown';
    ...
    switch($input_type) {
      ...
      case 'dropdown': ?>
        <select class="<?php echo $att_data['ui_class'] . $styled; ?>" tabindex="<?php echo $att_data['tabindex'] ?>" id="wpp_<?php echo $row_id; ?>_select" name="wpp_feps_data[<?php echo $att_data['slug']; ?>]">
          <?php if ( !isset( $att_data[ 'slug' ] ) || $att_data[ 'slug' ] !== 'property_type' ) : ?>
            <option value=""></option>
          <?php endif; ?>
          <?php foreach( $values as $k => $value ) { ?>
            <option value="<?php echo $k; ?>"<?php echo ( isset( $property[ $att_data['slug'] ] ) && $property[ $att_data['slug'] ]==$k ) ? ' selected="selected"' : ''; ?>><?php echo  apply_filters('wpp_stat_filter_' . $att_data['slug'], $value); ?></option>
          <?php } ?>
        </select>
        <?php
      break;
      ...
      case 'checkbox': ?>
        <input tabindex="<?php echo $att_data['tabindex'] ?>" type="checkbox" id="wpp_<?php echo $row_id; ?>_input"  name="wpp_feps_data[<?php echo $att_data['slug']; ?>]" class="<?php echo $att_data['required']=='on'?'required ':' '; echo $att_data['ui_class'].$styled; ?>" value="true" <?php echo ( isset( $property[$att_data['slug']] ) && $property[$att_data['slug']]==true) ? ' checked="checked"' : ''; ?> />
      <?php
      break;

      case 'textarea':
        ?>
        <textarea tabindex="<?php echo $att_data['tabindex'] ?>"  id="wpp_<?php echo $row_id; ?>_input" name="wpp_feps_data[<?php echo $att_data['slug']; ?>]" class="<?php echo $att_data['ui_class']; ?>"><?php echo !empty( $property[$att_data['slug']] ) ? $property[$att_data['slug']] : '';?></textarea>
      <?php
      break;
          ...
          <table class="ud_ui_dynamic_table widefat wpp_feps_sortable" use_random_row_id="true">
            <thead>
              <tr>
                ...
                <th class="wpp_plan_featured_col"><?php _e('Is Featured', 'wpp'); ?></th>
            ...
            <tfoot>
              <tr>
                <td colspan="8">
                  <input type="button" callback_function="wpp.ui.feps.on_added_row" class="wpp_add_row button-secondary" value="<?php _e('Add Subscription Plan','wpp') ?>" />
                </td>
              </tr>
            </tfoot>
          </table>
  ...
  static public function spc_options_general( $args = '' ) {
    ...
    <li>
      ...
      <label for="feps_credits_<?php echo $form_id; ?>"><?php _e('This is Sponsored Listing', 'wpp'); ?></label>
    </li>
  <?php
  }
  ...
  static public function feps_pay_now() {
    ...
    if ( wp_verify_nonce($_REQUEST['_wpnonce'], 'wpp_feps_pay_now') ) {
      ...
      if ( is_user_logged_in() ) {
        ...
        if ( !$property_id ) {
          $response[ 'message' ] = __('Something went wrong. Please, try again later. [Error 001]', 'wpp');
          die( json_encode( $response ) );
        }

        $form_id = get_post_meta( $property_id, FEPS_META_FORM, 1 );
        if ( !$form_id ) {
          $response[ 'message' ] = __('Something went wrong. Please, try again later. [Error 002]', 'wpp');
          die( json_encode( $response ) );
        }
        
        if( !self::set_subscription_plan( $property_id, $subscription_plan ) ) {
          $response[ 'message' ] = __('Something went wrong. Please, try again later. [Error 003]', 'wpp');
          die( json_encode( $response ) );
        }
        ...
        if ( $credits >= $amount ) {
          ...
          if ( wp_update_post( array( 'ID' => $property_id, 'post_status' => 'publish' ) ) ) {
            ...
            if ( update_user_meta($user->ID, FEPS_USER_CREDITS, $credits) ) {
              //** Success */
              $response[ 'success' ] = 1;
              $response[ 'message' ] = __( 'Thank you! Your payment has been successfully made.', 'wpp' );
              if( is_user_logged_in() ) {
                $response[ 'message' ] .= ' ' . sprintf( __( '<a href="%s">View %s</a>', 'wpp' ), WPP_F::base_url( FEPS_VIEW_PAGE ), WPP_F::property_label( 'plural' ) );
              }
              die( json_encode( $response ) );
            }
          }
          //** Error */
          $response[ 'message' ] = __('Something went wrong. Please, try again later. [Error 005]', 'wpp');
          die( json_encode( $response ) );
        }
        //** Error */
        $response[ 'message' ] = __('Not enough credits.', 'wpp');
        die( json_encode( $response ) );

      }
      //** Error */
      $response[ 'message' ] = __('You must be logged in to be able to do this action.', 'wpp');
      die( json_encode( $response ) );
    }
    //** Error */
    $response[ 'message' ] = __('Something went wrong. Please, try again. [Error 006]', 'wpp');
    die( json_encode( $response ) );
  }
  ...
  static public function filter_submit_button( $btn_label, $property, $form ) {
    ...
    switch (1){
      ...
      default:
        $btn_label = current_user_can('publish_wpp_properties')?__('Publish', 'wpp'):__('Submit', 'wpp');
    }
...
class MyFepsWidget extends WP_Widget {
  ...
  public function __construct() {
    parent::__construct( false, $name = __( 'FEPS Menu' ), array( 'description' => __( 'Shows FEPS Menu. It\'s available only for logged in users', 'wpp' ) ) );
  }
...
class MyFepsInfoWidget extends WP_Widget {
  ...
  public function __construct() {
    parent::__construct( false, $name = __( 'FEPS Information' ), array( 'description' => sprintf( __( 'Shows Information about current FEPS %1$s. It\'s available only for logged in users and shown only on Edit %1$s page.', 'wpp' ), WPP_F::property_label( 'singular' ) ) ) );
  }
