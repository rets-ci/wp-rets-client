...
class class_agents {
              ...
              <tr>
                <?php if( !empty( $agent_images[ 0 ][ 'src' ] ) ) : ?>
                  <td width="<?php echo $agent_photo_width; ?>"><?php
                    ...
                    if( @getimagesize( $agent_images[ 0 ][ 'src' ] ) ) { ?>
  ...
  static public function show_agents() {
    ...
    <h2><?php _e( 'Agents', 'wpp' ); ?>
      <a class="button add-new-h2" href="<?php echo self::get_link_edit( -1 ); ?>"><?php _e( 'Add New', 'wpp' ); ?></a></h2>
  ...
  static public function save_property( $post_id ) {
    ...
    if( current_user_can( self::$capability ) ) {
      ...
      if( !empty( $_REQUEST[ 'wpp_agents' ] ) && is_array( $_REQUEST[ 'wpp_agents' ] ) ) {
        foreach( $_REQUEST[ 'wpp_agents' ] as $agent_id ) {
          update_post_meta( $post_id, 'wpp_agents', $agent_id );
        }
      }
    }
    /*
    ...
    if( in_array( 'agent', $roles ) && !in_array( 'administrator', $roles ) ) {
      ...
      if( !in_array( $current_user->ID, $agents ) ) {
        update_post_meta( $post_id, 'wpp_agents', $current_user->ID );
      }
    }
  }
  ...
  static public function role_description( $description ) {
    $description = __( 'Agent can see only the properties which are assigned to him. Agent will be automatically assigned to the property which he will create.', 'wpp' );
    return $description;
  }
   ...
   * @param array $filters
   */
  static public function filter_get_search_filters( $filters ) {
    ...
    if( !isset( $filter_fields[ 'values' ] ) || count( $filter_fields[ 'values' ] ) <= 1 ) {
      return $filters;
    }
