...
class class_wpp_property_import {
    ...
    if( is_array( $working_ids ) && count( $working_ids ) ){
      ...
      foreach( $working_ids as $key => $row ){
        ...
        if( time() - (int) $time_started > 15 * 60 ){
          ...
          self::maybe_echo_log( 'This post still has time to complete.' );
        }
      }
    }else{
      $working_ids = array();
        ...
        if( $schedule[ 'source_type' ] == 'rets' ){
          ...
          foreach( (array) $photos as $image_count => $photo ) {
            ...
            try {
              ...
              $_unique_id = md5( ( isset( $photo[ 'Content-ID' ] ) ? $photo[ 'Content-ID' ] : '' ) .':' . ( isset( $photo[ 'Object-ID' ] ) ? $photo[ 'Object-ID' ] : '' ) );
              $image_exists = $wpdb->get_col( $wpdb->prepare( "SELECT post_id FROM {$wpdb->postmeta} WHERE meta_key = 'unique_id' AND meta_value = %s LIMIT 1", $_unique_id ) );
              if( $image_exists ) {
                continue;
              }
      ...
      if( !defined( 'XMLI_SYSTEM_COMMAND_CRON' ) ) {
        ...
        if( count( $identifier = array_intersect( array_keys( self::$static_request_map ), array_keys( $args ) ) ) ){
          ...
          foreach( $wpp_property_import[ 'schedules' ] as $id => $data ){
            if( $id == $to_find || $data[ 'hash' ] == $to_find ) {
  ...
  static public function handle_browser_import( $sch_id, $import_data ) {
    ...
    class_wpp_property_import::maybe_echo_log( sprintf( __( 'Starting ' . ( defined( 'WP_CLI' ) && WP_CLI ? 'CLI' : 'Browser' ) . '-Initiated Import: %1s. Using XML Importer %2s and WP-Property %3s.', 'wpp' ), $import_data['name'], WPP_XMLI_Version, WPP_Version ));
   ...
   *
   */
  static public function post_submitbox_misc_actions( $id ) {
  ...
  static public function maybe_debug_log( $message ) {
    if( is_callable( array( 'WPP_F', 'maybe_debug_log' ) ) ) {
      return WPP_F::maybe_debug_log( $message, 'xmli' );
    }
    return false;
  }
  ...
  static public function wpp_settings_help_tab() {
    ...
    wp_enqueue_script( 'wpp-settings-xmli', WPP_URL . 'js/wpp.admin.settings.xmli.js', array( 'jquery' ), WPP_Version );
   ...
   * Copyright 2010 - 2012 Usability Dynamics, Inc.
   */
  static public function jqueryui_widget_stylesheet_init() {
  ...
  static public function admin_init() {
    ...
    if( 
      isset( $wpp_property_import['settings']['allow_xml_uploads_via_media_uploader'] ) 
      && $wpp_property_import['settings']['allow_xml_uploads_via_media_uploader'] == 'true' 
    ) {
      add_filter( 'upload_mimes', array( 'class_wpp_property_import', 'add_upload_mimes' ) );
    }
            ...
            <tr <?php echo implode( '', $this_row_data ); ?> class="wpp_i_schedule_row" schedule_id="<?php echo $sch_id; ?>" import_title="<?php echo esc_attr( $sch['name'] ); ?>">
              <td class="post-title column-title">
                <ul>
                  ...
                  <li><?php _e( 'Cron Command:', 'wpp' ); ?> <span class="wpp_i_overview_special_data">php -q <?php echo $cron_path . ' do_xml_import ' . $sch['hash'] . ( is_multisite() ? " ".parse_url( site_url(), PHP_URL_HOST).parse_url( site_url(),PHP_URL_PATH) : '' ); ?></span></li>
                  ...
                  <li>
                    ...
                    <a href="<?php echo  site_url()."/?wpp_schedule_import=".$sch['hash']."&echo_log=true"; ?>" target="_blank" /><?php _e( 'Run Import in Browser', 'wpp' ); ?></a> |
      ...
      <td>
        ...
        <ul class="wpp_property_import_settings hidden">
          ...
          <li class="wpp_xi_advanced_setting" data-exclude_type="rets">
            <label class="description"><?php _e( 'Imported properties must have at least ','wpp' );?>
              <input type="text" id="wpp_i_minimum_images" class="wpp_xmli_enforce_integer" name="wpp_property_import[minimum_images]" value="<?php echo ( empty( $settings['minimum_images'] ) ? '' : $settings['minimum_images'] ); ?>"/><?php _e( ' valid image(s).','wpp' );?>
            </label>
          </li>
          
          <li class="wpp_xi_advanced_setting">
            <label class="description"><?php _e( 'Imported properties must have no more than ','wpp' );?>
          ...
          <li class="wpp_xi_advanced_setting">
            ...
            <label class="description" for="wpp_property_skip_images"><?php echo __( 'Skip images if images already downloaded.','wpp' );?></label>
          </li>
          ...
          <?php if( !defined( 'XMLI_SYSTEM_COMMAND_CRON' ) ) : ?>
            <li class="wpp_xi_advanced_setting">
              ...
              <label class="description" for="run_system_command_cron">
                <?php echo __( 'Run system command cron instead of PHP cron.', 'wpp' );?>
                <span class="description"><?php echo __( 'Attention, enable the option only if listings are not being published but added to <b>Draft status</b>.', 'wpp' ); ?></span>
              </label>
            </li>
          <?php endif; ?>
          ...
          <tr>
            <td colspan="3">
              ...
              <div class="alignright">
                ...
                <input type="button" <?php echo $save_button_id ?> class="button-primary" value="<?php _e( 'Save Configuration','wpp' ) ?>" />
    ...
    <tr class="wpp_i_import_actions">
      ...
      <td>
        <div class="wpp_i_import_actions_bar">
          ...
          <input type="button" id="wpp_i_preview_action" value="<?php _e( 'Preview Import', 'wpp' ); ?>" class="button-secondary" />
          <input type="button" id="wpp_i_do_full_import" value="<?php _e( 'Process Full Import', 'wpp' ); ?>" class="button-secondary" />
  ...
  static public function get_total_attribute_array() {
    ...
    $data = array(
      'property_type' => __( 'Property Type', 'wpp' ),
      'wpp_agents' => __( 'Property Agent', 'wpp' ),
      'wpp_gpid' => __( 'Global Property ID', 'wpp' ),
      'display_address' => __( 'Display Address', 'wpp' ),
    );
  ...
  static public function import_object( $data, $schedule_id, $counter = false ) {
    ...
    if( isset( $schedule_settings['revalidate_addreses_on_completion'] ) && $schedule_settings['revalidate_addreses_on_completion'] == 'on' ) {
      ...
      $validation_statuses = array(
        'skipped' => __( 'Address validation was skipped because address has already been validated.', 'wpp' ),
        ...
        'over_query_limit' => __( 'Address validation has failed because the Google service has denied the request ( OVER QUERY LIMIT ).', 'wpp' ),
        'failed' => __( 'Address validation has failed.', 'wpp' ),
