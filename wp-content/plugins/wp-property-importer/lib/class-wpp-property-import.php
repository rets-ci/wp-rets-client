 ...
 *
 * @update 4.0.4
 * - Implemented wp-cli functionality for XMLI (experts only) - use 'wp-cli xmli' to get commands
 * - Allows for cron jobs to be set with system cron instead of wp_cron
 * - Updated image pulling processing to not fail in backend
 * - Better error handling for image pulling
 *
 * @updated 4.0
...
class class_wpp_property_import {
  ...
  static public function post_submitbox_misc_actions( $id ) {
    ...
    if( !empty( $import_time ) ) {
      ...
      <div class="misc-pub-section xml_import_time misc-pub-section-last">
        <span class="wpp_i_time_stamp"><?php printf( __( 'Imported on: <b>%1$s</b> <a href="%2$s" title="%3$s">Importer</a>', ud_get_wpp_importer()->domain ),$import_time, $import_url, $import_name ); ?> <b></b></span>
  ...
  static public function wpp_settings_help_tab() {
    ...
    <div class='wpp_settings_block'><?php _e( 'Look up XML import history.',ud_get_wpp_importer()->domain ) ?>
      <input type="button" value="<?php _e( 'Show XML Import History',ud_get_wpp_importer()->domain ) ?>" id="wpp_ajax_show_xml_imort_history">
  ...
  static public function page_main() {
    ...
    <div class="wrap">
            ...
            <tr <?php echo implode( '', $this_row_data ); ?> class="wpp_i_schedule_row" schedule_id="<?php echo $sch_id; ?>" import_title="<?php echo esc_attr( $sch['name'] ); ?>">
              <td class="post-title column-title">
                <ul>
                  ...
                  <li><?php _e( 'Source URL:', ud_get_wpp_importer()->domain ); ?> <span class="wpp_i_overview_special_data"><?php echo $sch['url']; ?></span></li>
                  <li><?php _e( 'Cron Command:', ud_get_wpp_importer()->domain ); ?> <span class="wpp_i_overview_special_data">php -q <?php echo $cron_path . ' do_xml_import ' . $sch['hash'] . ( is_multisite() ? " ".parse_url( site_url(), PHP_URL_HOST).parse_url( site_url(),PHP_URL_PATH) : '' ); ?></span></li>
      ...
      <div class="wpp_import_import_schedule wpp_import_overview_page_element">
        ...
        <?php _e( "Import Schedule", ud_get_wpp_importer()->domain ); ?>: <input name="wpp_import[import_schedule]" type="file" />
      ...
      <td>
        ...
        <table id="wpp_property_import_attribute_mapper" auto_increment="true" class="ud_ui_dynamic_table widefat">
          ...
          <tr>
            ...
            <th style="width: auto;"><?php echo __( 'XPath Rule',ud_get_wpp_importer()->domain );?></th>
          </tr>
          </thead>
          <tbody>
          ...
          <tr>
            <td colspan="3">
              <div class="alignleft">
                ...
                <input type="button" class="wpp_add_row button-secondary" value="<?php _e( 'Add Row',ud_get_wpp_importer()->domain ) ?>" />
