...
/**
 *
 * - wp_rets_mapper_updates transient used for state
 *
 * - pre_set_site_transient_update_plugins
 * - site_transient_update_plugins - on every run
 */

add_filter( 'site_transient_update_plugins', array( 'Rets_Mapper_Updater', 'update_check_handler' ), 10 );
add_filter( 'upgrader_process_complete', array( 'Rets_Mapper_Updater', 'upgrader_process_complete' ), 10, 2 );

class Rets_Mapper_Updater {
   ...
   * @return bool
   */
  static public function rabbit_auto_update_specific_plugins( $update, $item ) {
    // Array of plugin slugs to always auto-update

    $plugins = array( 'wp-rets-mapper' );
  ...
  static public function get_update_check_result() {
    ...
    if( get_site_transient( 'wp_rets_mapper_updates' ) && !isset( $_GET[ 'force-check' ] ) ) {

      $_transient = get_site_transient( 'wp_rets_mapper_updates' );
    ...
    $_products = array( 'wp-rets-mapper' => 'wp-rets-mapper/wp-rets-mapper.php' );
    ...
    if( isset( $_detail ) ) {
      $_transient_result = set_site_transient( 'wp_rets_mapper_updates', array( 'data' => $_detail, 'cached' => true ), 1800 );
    }
  ...
  static public function upgrader_process_complete() {
    delete_site_transient( 'wp_rets_mapper_updates' );
  }
