...
class Client {
	...
	public function apiGet($path, $options = array()) {
		...
		$url  = (strpos($path, 'https://') === 0) ? $path : self::API_URL . $path. '?access_token=' . urlencode($this->_state->token->data->access_token);
	...
	public function apiPost($path, $data = null) {
		$url  = (strpos($path, 'https://') === 0) ? $path : self::API_URL . $path;
