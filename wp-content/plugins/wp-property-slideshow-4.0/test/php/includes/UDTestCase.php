...
class UD_Plugin_WP_UnitTestCase extends WP_UnitTestCase {
  ...
  function setUp() {
    ...
    if( file_exists( $this->root_dir . '/wp-property-slideshow.php' ) ) {
      include_once( $this->root_dir . '/wp-property-slideshow.php' );
    }
    if( !class_exists( '\UsabilityDynamics\WPP\Slideshow_Bootstrap' ) ) {
      $this->fail( 'Plugin is not available.' );
    }
    $this->instance = \UsabilityDynamics\WPP\Slideshow_Bootstrap::get_instance();
  }
