...
wpp.xmli = jQuery.extend({
  ...
  ready: function ready() {
    ...
    jQuery( '#check_all' ).live( 'click', function () {
      if ( jQuery(this).is(':checked') ) {
        jQuery( '[name^="wpp_property_import[map]"]:checkbox' ).prop('checked', true);
      } else {
        jQuery( '[name^="wpp_property_import[map]"]:checkbox' ).prop('checked', false);
      }
    } )
...
//** Switch sorting direction */
    ...
    /** MATCHES COLUMN ON EDIT SCHEDULE PAGE */

    /**
     * Toogle Matches Section
     */
    jQuery( 'a.toggle_advanced_settings' ).live( 'click', function() {
      var wrap = jQuery(this).parents('.matches-section').find( 'ul.advanced-options' );
      if( wrap.is( ':hidden' ) ) {
        wrap.show();
        jQuery(this).html( wpp.strings.xmli.hide_matches );
      } else {
        wrap.hide();
        jQuery(this).html( wpp.strings.xmli.show_matches );
      }
      return false;
    } );
     ...
     * Add Match
     */
    jQuery( '.add_match_row' ).live( 'click', function() {
      var name_prefix = jQuery( this ).closest( 'tr').find( 'select.wpp_import_attribute_dropdown').attr( 'name').replace( '[wpp_attribute]', ''),
        key = Math.floor( Math.random() * ( 9999 - 1001 + 1) ) + 1001,
        el = '';
      el += '<li class="clearfix"><span>';
      el += '<input placeholder="' + wpp.strings.xmli.matches_via_comma + '" type="text" name="' + name_prefix + '[matches][' + key + '][match]" value="" /></span>';
      el += ' <span><input placeholder="' + wpp.strings.xmli.value + '" type="text" name="' + name_prefix + '[matches][' + key + '][value]" value=""></span>';
      el += ' <a href="javascript:;" class="button remove_match_row">' + wpp.strings.xmli.remove + '</a>';
      el += '</li>';
      jQuery( this).parents( 'ul.matches' ).find( 'ul.list').append( el );
      var counter = jQuery( this ).parents('.matches-section').find( '.counter'),
        length = jQuery( this ).parents('.matches-section').find( '.list li').length;
      counter.html( length );
    } );
     ...
     * Remove Match
     */
    jQuery( '.remove_match_row' ).live( 'click', function() {
      var section = jQuery( this ).parents('.matches-section');
      jQuery( this ).closest( 'li' ).remove();
      var counter = section.find( '.counter')
          length = section.find( '.list li').length;
      counter.html( length );
    } );
  ...
  /**
   * Remove all legacy matches from just added row
   *
   * @param row
   */
  ...
  /**
   * Show/Hide Alternative CRON options
   *
   */
  toggle_cron_options: function() {
    if ( jQuery( "#wpp_property_import_alt_cron_enabled" ).is(':checked')) {
      jQuery( ".wpp_property_import_alt_cron_options" ).show();
    } else {
      jQuery( ".wpp_property_import_alt_cron_options" ).hide();
    }
  },
  ...
  run_on_import_ui_display: function run_on_import_ui_display ( response ) {
    ...
    /* Show/Hide CRON options */
    jQuery( "#wpp_property_import_alt_cron_enabled" ).click( function(){
      wpp.xmli.toggle_cron_options();
    } );
    wpp.xmli.toggle_cron_options();
