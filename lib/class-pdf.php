...
class class_wpp_pdf_flyer {
    ...
    <div class='wrap'>
      ...
      <table class="ud_ui_dynamic_table widefat form-table" allow_random_slug="true">
                ...
                <td>
                  <ul>
                    <li>
                      ...
                      <?php echo WPP_F::draw_attribute_dropdown( "name=wpp_flyer_list_settings[{$slug}][group_by]&selected={$list['group_by']}", array( 'property_type' => __( 'Property Type', 'wpp' ) ) ); ?>
                    </li>
        ...
        <div class="wpp_settings_block" style="margin: 10px 0;">
          ...
          <br/><input style="display:none;" type="button" id="wpp_ajax_regenerate_all_flyers_close" value="<?php _e( 'Refresh Page', 'wpp' ); ?>" onClick="window.location.reload();">
        ...
        <div class='widefat' style="margin:10px 0;padding:10px;width:auto;">
          <p><span class="description"><?php _e( 'Shortcode examples:', 'wpp' ); ?></span></p>
          <ul>
            <li><span class="description"><b>[wpp_pdf_list name='list_name']</b> - <?php _e( 'Returns a formatted HTML link to the PDF List.', 'wpp' ); ?></span></li>
            <li><span class="description"><b>[wpp_pdf_list name='list_name' title='PDF Flyer']</b> - <?php _e( 'Returns a html link to the PDF List with custom title.', 'wpp' ); ?></span></li>
            <li><span class="description"><b>[wpp_pdf_list name='list_name' urlonly='yes']</b> -  <?php _e( 'Returns the raw URL to the PDF List (for use in custom html).', 'wpp' ); ?></span></li>
            <li><span class="description"><b>[wpp_pdf_list name='list_name' class='custom_css_class']</b> - <?php _e( 'For use with a custom CSS class.', 'wpp' ); ?></span></li>
            <li><span class="description"><b>[wpp_pdf_list name='list_name' image='url_to_custom_image']</b> - <?php _e( 'Returns url_to_custom_image with a link to the PDF List.', 'wpp' ); ?></span></li>
          </ul>
        </div>
  ...
  static function template_redirect() {
    ...
    if( $_REQUEST[ 'wpp_flyer_create' ] == 'true' && $post->ID ) {
      ...
      if( self::flyer_exists( $post->ID ) ) {
        wp_redirect( get_pdf_flyer_permalink( $post->ID ) );
        exit;
      }
                ...
                <tr>
                    <td style="text-align:justify;"><a href="<?php echo $property[ 'permalink' ] ?>"><img width="<?php echo( $wpp_pdf_flyer[ 'second_photo_width' ] - 20 ); ?>" src="<?php echo $wpp_pdf_flyer[ 'qr_code' ]; ?>" alt=""/></a>
                      <?php if( !empty( $wpp_pdf_flyer[ 'qr_code_note' ] ) ) : ?>
                        <br/><br/><span class="pdf-note">Scan the code above with your mobile device or click it to go directly to the <?php echo get_bloginfo(); ?>
                          website for <?php echo $property[ 'post_title' ]; ?></span>
                      <?php endif; ?>
                    </td>
                </tr>
                </table>
  ...
  <tr valign="top">
    ...
    <tr valign="top">
      ...
      <td>
        <?php // wpp_tcpdf_get_HTML_font_list( "name=wpp_settings[configuration][feature_settings][wpp_pdf_flyer][setfont]&selected={$wpp_pdf_flyer['setfont']}" ); ?>
        <br>
