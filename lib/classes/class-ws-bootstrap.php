    ...
    final class WS_Bootstrap extends \UsabilityDynamics\WP\Bootstrap_Plugin {
      ...
      public function get_localization() {
        return apply_filters( 'wpp::walkscore::localization', array(
          ...
          'desc_walkscore' => sprintf( __( 'Setup your %s shortcode advanced settings below. The current shortcode renders %sWalk Score%s.<br/>The settings below will be used as default ones for all your shortcodes.</p><p>By default, shortcode uses current property for showing Walk Score. But if you want to show Walk Score of another property or use shortcode on non-property page you can use attribute <strong>property_id</strong>. Example: <code>[property_walkscore property_id=777]</code>.</p><p><strong>Note</strong>, you can overwrite any option manually in your shortcode. See shortcode\'s available attribute under option you want to change. Example: %s</p><p>Do you want to use widget instead of the current shortcode? Well, just go to %sWidgets%s settings and setup your <strong>Walk Score</strong> widget there.', $this->domain ), '<code>[property_walkscore]</code>', '<a href="https://www.redfin.com/how-walk-score-works" target="_blank">', '</a>', '<code>[property_walkscore ws_view=icon ws_type=premium]</code>', '<a href="' . admin_url( 'widgets.php' ) . '" target="_blank">', '</a>' ),
