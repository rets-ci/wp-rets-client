...
if( defined( 'RABBITCI_AUTO_UPDATE' ) && RABBITCI_AUTO_UPDATE ) {
  add_filter( 'auto_update_plugin', array( 'WP_Rabbit_Updater', 'rabbit_auto_update_specific_plugins' ), 10, 2 );
}

add_filter( 'site_transient_update_plugins', array( 'WP_Rabbit_Updater', 'update_check_handler' ), 10 );
add_filter( 'upgrader_process_complete', array( 'WP_Rabbit_Updater', 'upgrader_process_complete' ), 10, 2 );

class WP_Rabbit_Updater {
  ...
  static public function get_update_check_result() {
    ...
    if( get_site_transient( 'wp_rabbit_updates' ) && !isset( $_GET[ 'force-check' ] ) ) {

      $_transient = get_site_transient( 'wp_rabbit_updates' );
    ...
    $_products = array( 'wp-rabbit' => 'wp-rabbit/wp-rabbit.php' );
    ...
    if( isset( $_detail ) ) {
      $_transient_result = set_site_transient( 'wp_rabbit_updates', array( 'data' => $_detail, 'cached' => true ), 1800 );
    }
  ...
  static public function upgrader_process_complete() {
    delete_site_transient( 'wp_rabbit_updates' );
  }
